...
function ChatApp(): React.JSX.Element {
  ...
  useEffect(() => {
    void window.souldesk.getSnapshot().then(setSnapshot);
    const offSnapshot = window.souldesk.onSnapshot(setSnapshot);
    const offDelta = window.souldesk.onChatDelta((event) => {
  ...
  async function send(): Promise<void> {
    ...
    try { setRequestId(await window.souldesk.sendMessage(content)); } catch (reason) { setError(reason instanceof Error ? reason.message : "发送失败"); }
  }
  ...
  return <main className="chat-shell">
    <header className="chat-titlebar">
      ...
      <div className="title-actions">
        <button className="icon-button" title="设置" aria-label="打开设置" onClick={() => void window.souldesk.openManager()}><Settings size={17}/></button>
    ...
    <footer className="composer">
      ...
      <div className="composer-actions">
        <button className="icon-button" title="清空记录" aria-label="清空记录" onClick={() => void window.souldesk.clearMessages()}><Trash2 size={17}/></button>
        {requestId ? <button className="send-button stop" title="停止" aria-label="停止生成" onClick={() => void window.souldesk.stopMessage(requestId)}><Square size={15}/></button> : <button className="send-button" title="发送" aria-label="发送消息" disabled={!text.trim()} onClick={() => void send()}><Send size={17}/></button>}
      </div>
    </footer>
  </main>;
}
