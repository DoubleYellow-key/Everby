...
function ChatApp(): React.JSX.Element {
  ...
  return <main className="chat-shell">
    ...
    <section className="messages" aria-live="polite">
      ...
      {pendingMessage && <div className={`message user optimistic ${pendingMessage.failed ? "failed" : ""}`}>{pendingMessage.attachments.length > 0 && <div className="message-images">{pendingMessage.attachments.map((image) => <img key={image.id} src={image.dataUrl} alt={image.name}/>)}</div>}<span>{pendingMessage.content}</span>{pendingMessage.failed && <small>发送失败</small>}</div>}
