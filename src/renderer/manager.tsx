...
import { Bot, Brain, Check, CheckCircle2, ChevronRight, Eye, ListTodo, MessageCircle, Palette, Play, Plus, Save, ShieldCheck, Sparkles, Sun, Trash2, UserRound } from "lucide-react";
...
function ManagerApp(): React.JSX.Element {
  ...
  return <div className="manager-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><Sun size={21}/></div><div><strong>Everby</strong><span>常伴你的桌面智能体</span></div></div>
    ...
    <main className="settings-main">
      {tab === "companion" && <section><PageTitle title="陪伴状态" description={`控制桌面上的 ${activePet.name} 和对话入口。`}/><div className="status-band"><div><span>当前角色</span><strong>{activePet.name}</strong><small>{snapshot.model.configured ? `已连接 ${snapshot.model.model}` : "离线动作模式"}</small></div><button className="primary" onClick={() => void window.everby.openChat()}><Play size={17}/>开始对话</button></div><SettingRow title="显示桌宠" detail="隐藏后仍可从托盘恢复"><Toggle label="显示桌宠" checked={snapshot.settings.visible} onChange={(visible) => void window.everby.updateSettings({ visible })}/></SettingRow><SettingRow title="暂停日常动作" detail="保留点击和对话响应"><Toggle label="暂停日常动作" checked={snapshot.settings.paused} onChange={(paused) => void window.everby.updateSettings({ paused })}/></SettingRow><SettingRow title="主动陪伴" detail="遵守免打扰时间和每日频率"><Toggle label="主动陪伴" checked={snapshot.settings.proactiveEnabled} onChange={(proactiveEnabled) => void window.everby.updateSettings({ proactiveEnabled })}/></SettingRow></section>}
