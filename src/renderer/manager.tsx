...
import { Bot, Brain, Check, CheckCircle2, ChevronRight, Eye, ListTodo, MessageCircle, PackagePlus, Palette, Play, Plus, Save, ShieldCheck, Sparkles, Sun, Trash2, UserRound } from "lucide-react";
...
function ManagerApp(): React.JSX.Element {
  ...
  async function saveEmbedding(form: FormData): Promise<void> {
    const patch = Object.fromEntries(form) as unknown as Partial<EmbeddingSettings> & { apiKey?: string }; await window.souldesk.updateEmbedding(patch); setStatus("Embedding 设置已保存");
  }
  ...
  async function createTodo(form: FormData): Promise<void> {
    await window.souldesk.createTodo({
