...
import { Bot, Brain, Check, CheckCircle2, ChevronRight, Coffee, Eye, Focus, ListTodo, MessageCircle, Palette, Play, Plus, Save, ShieldCheck, Sparkles, Sun, TimerOff, Trash2, UserRound } from "lucide-react";
...
function ModeControls({ snapshot, setStatus }: { snapshot: AppSnapshot; setStatus: (value: string) => void }): React.JSX.Element {
  ...
  const title = mode.mode === "focus" ? "专注中" : mode.mode === "rest" ? "休息中" : "常态陪伴";
  async function start(next: "focus" | "rest", duration: number): Promise<void> {
    await window.everby.startActionMode(next, duration);
    setStatus(`已开始 ${duration} 分钟${next === "focus" ? "专注" : "休息"}`);
  }
  return <div className="mode-control-panel">
    <div className="mode-current"><span>{mode.mode === "focus" ? <Focus size={19}/> : mode.mode === "rest" ? <Coffee size={19}/> : <Sparkles size={19}/>}</span><div><strong>{title}</strong><small>{mode.mode === "normal" ? "动作按常态活跃度自然轮换" : `剩余 ${minutes}:${String(seconds).padStart(2, "0")}`}</small></div>{mode.mode !== "normal" && <button className="secondary" onClick={() => void window.everby.stopActionMode().then(() => setStatus("已回到常态"))}><TimerOff size={16}/>提前结束</button>}</div>
    <div className="mode-shortcuts"><div><span><Focus size={16}/>专注</span>{[25, 45, 60].map((duration) => <button key={duration} className="secondary" onClick={() => void start("focus", duration)}>{duration} 分钟</button>)}</div><div><span><Coffee size={16}/>休息</span>{[5, 10, 15].map((duration) => <button key={duration} className="secondary" onClick={() => void start("rest", duration)}>{duration} 分钟</button>)}</div></div>
  </div>;
}
