...
import { Bot, Brain, Check, CheckCircle2, Eye, ListTodo, MessageCircle, PackagePlus, Palette, Play, Plus, Save, ShieldCheck, Sparkles, Sun, TimerOff, Trash2, UserRound } from "lucide-react";
import type { AppSnapshot, EmbeddingSettings, ModelSettings, PersonaProfile, PetSummary, VisionSettings } from "../shared/contracts";
...
type NavItem = { id: Tab; label: string; icon: React.ComponentType<{ size?: number }> };
const navGroups: Array<{ label: string; items: NavItem[] }> = [
  { label: "桌面", items: [{ id: "companion", label: "陪伴", icon: MessageCircle }, { id: "plans", label: "计划", icon: ListTodo }] },
  { label: "角色", items: [{ id: "persona", label: "角色", icon: UserRound }, { id: "motions", label: "动作", icon: Sparkles }, { id: "memory", label: "记忆", icon: Brain }] },
  { label: "系统", items: [{ id: "model", label: "模型", icon: Bot }, { id: "privacy", label: "隐私", icon: ShieldCheck }, { id: "appearance", label: "外观", icon: Palette }] }
];
...
function ManagerApp(): React.JSX.Element {
  ...
  async function saveVision(form: FormData): Promise<void> {
    const patch = Object.fromEntries(form) as unknown as Partial<VisionSettings> & { apiKey?: string }; await window.everby.updateVision(patch); setStatus("图片理解模型已保存");
  }
  ...
  async function deletePet(pet: PetSummary): Promise<void> {
    const confirmed = confirm(`确定删除“${pet.name}”吗？\n\n将删除本机 Petdex 角色文件，并清除它在 Everby 中的对话、计划、记忆、人设和动作状态。此操作不可撤销。`);
    if (!confirmed) return;
    try {
      await window.everby.deletePet(pet.id);
      setStatus(`${pet.name} 已删除`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "角色删除失败");
    }
  }
  ...
  return <div className="manager-shell">
    <header className="window-topbar"><span className="window-topbar-title">Everby</span></header>
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><Sun size={20}/></div><div><strong>Everby</strong><span>常伴你的桌面宠物</span></div></div>
      <nav aria-label="设置导航">{navGroups.map((group) => <div className="nav-group" key={group.label}><span className="nav-group-label">{group.label}</span>{group.items.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? "active" : ""} onClick={() => { setTab(id); setStatus(""); }}><Icon size={18}/><span>{label}</span></button>)}</div>)}</nav>
      <div className="sidebar-divider" aria-hidden="true"/>
      <div className="sidebar-foot"><span className="companion-thumb" style={{ backgroundImage: `url(${activePet.sheetUrl})` }}/><div className="companion-copy"><strong>{activePet.name}</strong><span><span className="presence-dot"/>正在桌面陪伴</span></div></div>
    </aside>
    <main className="settings-main">
      {tab === "companion" && <section><PageTitle title="陪伴状态" description={`控制桌面上的 ${activePet.name}、状态计时和对话入口。`}/><div className="status-band"><div><span>当前角色</span><strong>{activePet.name}</strong><small>{snapshot.model.configured ? `已连接 ${snapshot.model.model}` : "离线动作模式"}</small></div><button className="primary" onClick={() => void window.everby.openChat()}><Play size={17}/>开始对话</button></div><ModeControls snapshot={snapshot} setStatus={setStatus}/><div className="card card-rows"><SettingRow title="显示桌宠" detail="隐藏后仍可从托盘恢复"><Toggle label="显示桌宠" checked={snapshot.settings.visible} onChange={(visible) => void window.everby.updateSettings({ visible })}/></SettingRow><SettingRow title="暂停背景动作" detail="保留提醒、点击、对话和拖拽反馈"><Toggle label="暂停背景动作" checked={snapshot.settings.paused} onChange={(paused) => void window.everby.updateSettings({ paused })}/></SettingRow><SettingRow title="主动陪伴" detail="遵守免打扰时间和每日频率"><Toggle label="主动陪伴" checked={snapshot.settings.proactiveEnabled} onChange={(proactiveEnabled) => void window.everby.updateSettings({ proactiveEnabled })}/></SettingRow></div></section>}
      {tab === "plans" && <section><PageTitle title="计划与提醒" description="管理本地清单、截止时间和提醒。"/><div className="card"><form className="todo-form" action={(form) => void createTodo(form)}><Field label="计划内容" wide><input name="title" required maxLength={160} placeholder="例如：整理本周进度"/></Field><Field label="截止时间"><input name="dueAt" type="datetime-local"/></Field><Field label="提醒时间"><input name="remindAt" type="datetime-local"/></Field><Field label="重复"><select name="repeat" defaultValue="none"><option value="none">不重复</option><option value="daily">每天</option></select></Field><Field label="备注"><input name="notes" maxLength={500}/></Field><button className="primary form-submit"><Plus size={17}/>添加计划</button></form></div><div className="card card-rows plan-controls"><SettingRow title="定时提醒" detail="到点后显示系统通知和桌宠气泡"><Toggle label="定时提醒" checked={snapshot.settings.remindersEnabled} onChange={(remindersEnabled) => void window.everby.updateSettings({ remindersEnabled })}/></SettingRow><SettingRow title="AI 清单关注" detail="低频查看临近或逾期项目，并结合时间与已授权的应用名称回应"><Toggle label="AI 清单关注" checked={snapshot.settings.taskAssistantEnabled} onChange={(taskAssistantEnabled) => void window.everby.updateSettings({ taskAssistantEnabled })}/></SettingRow></div><div className="card list-card"><div className="todo-list" aria-label="计划清单">{snapshot.todos.length === 0 ? <Empty icon={CheckCircle2} title="清单为空" detail="可以在这里添加，也可以直接在对话中告诉桌面伙伴。"/> : snapshot.todos.map((todo) => <div className={`todo-row ${todo.completedAt ? "completed" : ""}`} key={todo.id}><input type="checkbox" aria-label={`完成 ${todo.title}`} checked={todo.completedAt !== null} onChange={(event) => void window.everby.updateTodo(todo.id, { completed: event.target.checked })}/><div><strong>{todo.title}</strong><span>{todoSchedule(todo)}</span>{todo.notes && <small>{todo.notes}</small>}</div><button className="icon-button danger" title="删除计划" aria-label={`删除 ${todo.title}`} onClick={() => void window.everby.deleteTodo(todo.id)}><Trash2 size={17}/></button></div>)}</div></div></section>}
      {tab === "persona" && <section><PageTitle title="角色与人设" description="选择本机已安装的 Petdex 角色，每个角色拥有独立的人设、对话和记忆。"/><div className="pet-import-row"><button className="primary" onClick={() => void importPet()}><PackagePlus size={17}/>导入角色</button><small>支持 Petdex 角色文件夹或 .zip 压缩包，导入后自动切换</small></div><div className="pet-picker" role="listbox" aria-label="桌宠角色">{snapshot.pets.map((pet) => <div key={pet.id} className={`pet-option ${pet.id === snapshot.activePetId ? "active" : ""}`}><button type="button" role="option" aria-selected={pet.id === snapshot.activePetId} className="pet-select-target" onClick={() => void selectPet(pet.id, pet.name)}><span className="pet-thumbnail" style={{ backgroundImage: `url(${pet.sheetUrl})` }}/><span className="pet-copy"><strong>{pet.name}</strong><small>{pet.source === "petdex" ? "Petdex 已安装" : "Everby 内置角色"}</small></span>{pet.id === snapshot.activePetId && <span className="pet-selected" title="当前角色"><Check size={17}/></span>}</button>{pet.source === "petdex" && <button type="button" className="pet-delete icon-button danger" title={`删除 ${pet.name}`} aria-label={`删除 ${pet.name}`} onClick={() => void deletePet(pet)}><Trash2 size={16}/></button>}</div>)}</div><form key={snapshot.persona.petId} className="form-grid card" action={(form) => void savePersona(form)}><Field label="名字"><input name="name" defaultValue={snapshot.persona.name}/></Field><Field label="对你的称呼"><input name="userAddress" defaultValue={snapshot.persona.userAddress}/></Field><Field label="角色背景" wide><textarea name="background" defaultValue={snapshot.persona.background} rows={4}/></Field><Field label="说话风格" wide><textarea name="speakingStyle" defaultValue={snapshot.persona.speakingStyle} rows={3}/></Field><Field label="行为边界" wide><textarea name="boundaries" defaultValue={snapshot.persona.boundaries} rows={3}/></Field><button className="primary form-submit"><Save size={17}/>保存角色</button></form></section>}
      {tab === "model" && <section><PageTitle title="模型连接" description="聊天、图片理解和 Embedding 可使用不同的 OpenAI-compatible 服务，凭据分别加密保存。"/><div className="status-band flat"><div><span>智能体状态</span><strong>{snapshot.agentStatus === "ready" ? "已就绪" : snapshot.agentStatus === "degraded" ? "降级运行" : "尚未配置"}</strong><small>{snapshot.agentCapabilities.toolCalling ? `工具调用可用${snapshot.agentCapabilities.vision ? "，识图工具可用" : ""}` : "陪伴聊天可用时，待办、识图与自动记忆工具将停用"}</small></div></div><form className="form-grid model-form card" action={(form) => void saveModel(form)}><h2>聊天模型</h2><Field label="Chat API Base URL" wide><input name="baseUrl" defaultValue={snapshot.model.baseUrl} placeholder="https://api.openai.com/v1"/></Field><Field label="聊天模型"><input name="model" defaultValue={snapshot.model.model}/></Field><Field label="Temperature"><input name="temperature" type="number" min="0" max="2" step="0.1" defaultValue={snapshot.model.temperature}/></Field><Field label="Chat API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.model.configured ? "已安全保存，留空则不修改" : "输入 API Key"}/></Field><div className="form-actions"><button className="primary"><Save size={17}/>保存连接</button><button type="button" className="secondary" onClick={() => void window.everby.testModel().then((result) => setStatus(result.message))}>探测能力</button></div></form><form className="form-grid model-form card" action={(form) => void saveVision(form)}><h2>图片理解模型</h2><p className="form-description">作为聊天智能体的 inspect_image 工具，仅在本轮附加图片时按需调用。</p><Field label="Vision API Base URL" wide><input name="baseUrl" defaultValue={snapshot.vision.baseUrl}/></Field><Field label="视觉模型"><input name="model" defaultValue={snapshot.vision.model}/></Field><Field label="Vision API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.vision.configured ? "已安全保存，留空则不修改" : "输入独立 API Key"}/></Field><div className="form-actions"><button className="primary"><Save size={17}/>保存图片理解模型</button><button type="button" className="secondary" onClick={() => void window.everby.testVision().then((result) => setStatus(result.message))}>测试识图</button></div></form><form className="form-grid model-form card" action={(form) => void saveEmbedding(form)}><h2>Embedding 模型</h2><Field label="Embedding API Base URL" wide><input name="baseUrl" defaultValue={snapshot.embedding.baseUrl}/></Field><Field label="Embedding 模型"><input name="model" defaultValue={snapshot.embedding.model}/></Field><Field label="Embedding API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.embedding.configured ? "已安全保存，留空则不修改" : "输入独立 API Key"}/></Field><button className="primary form-submit"><Save size={17}/>保存 Embedding</button></form></section>}
      {tab === "motions" && <MotionSettings snapshot={snapshot} setStatus={setStatus}/>}
      {tab === "memory" && <section><PageTitle title="长期记忆" description="事实与向量只保存在这台电脑，并按角色隔离。"/><div className="card card-rows"><SettingRow title="当前会话" detail={`保存 ${snapshot.messages.length} 条消息；清空会话不会删除长期记忆`}><button className="secondary danger-text" onClick={() => void window.everby.clearMessages()}><Trash2 size={16}/>清空会话</button></SettingRow></div><div className="card list-card"><div className="memory-list">{snapshot.memories.length === 0 ? <Empty icon={Brain} title="还没有长期记忆" detail="明确说“记住”时会立即保存，稳定事实也可由后台整理。"/> : snapshot.memories.map((memory) => <div className="memory-row" key={memory.id}><div><strong>{memory.subject === "pet" ? "角色自身" : "用户"} · {memory.type} · {Math.round(memory.confidence * 100)}%</strong><span>{memory.content}</span><small>{new Date(memory.createdAt).toLocaleString()} · {memory.indexed ? "向量已索引" : "等待索引"}</small></div><button className="icon-button danger" title="删除记忆" onClick={() => void window.everby.deleteMemory(memory.id)}><Trash2 size={17}/></button></div>)}</div></div>{snapshot.memories.length > 0 && <div><button className="secondary danger-text" onClick={() => { if (confirm("确定清空当前角色的全部长期记忆吗？此操作不可撤销。")) void window.everby.clearMemories(); }}><Trash2 size={16}/>清空长期记忆</button></div>}</section>}
      {tab === "privacy" && <section><PageTitle title="隐私" description="上下文采集默认关闭，并且不会读取窗口内容。"/><div className="card card-rows"><SettingRow title="感知前台应用" detail="只读取应用名称；不保存标题、URL、文件名或截图"><Toggle label="感知前台应用" checked={snapshot.settings.activeAppEnabled} onChange={(activeAppEnabled) => void window.everby.updateSettings({ activeAppEnabled })}/></SettingRow></div><div className="privacy-note"><Eye size={19}/><p><strong>当前采集边界</strong><span>时间、系统空闲状态，以及你主动开启后的前台应用名称。锁屏时停止采集和主动消息。</span></p></div></section>}
      {tab === "appearance" && <section><PageTitle title="外观" description={`调整 ${activePet.name} 在桌面上的尺寸和层级。`}/><div className="card card-rows"><SettingRow title="始终置顶" detail={`让 ${activePet.name} 保持在普通窗口上方`}><Toggle label="始终置顶" checked={snapshot.settings.alwaysOnTop} onChange={(alwaysOnTop) => void window.everby.updateSettings({ alwaysOnTop })}/></SettingRow><label className="range-row"><span><strong>桌宠大小</strong><small>{Math.round(snapshot.settings.scale * 100)}%</small></span><input type="range" min="0.5" max="2" step="0.05" value={snapshot.settings.scale} onChange={(event) => void window.everby.updateSettings({ scale: Number(event.target.value) })}/></label></div></section>}
