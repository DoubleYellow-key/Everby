...
function ManagerApp(): React.JSX.Element {
  ...
  return <div className="manager-shell">
    ...
    <main className="settings-main">
      ...
      {tab === "model" && <section><PageTitle title="模型连接" description="聊天、图片理解和 Embedding 可使用不同的 OpenAI-compatible 服务，凭据分别加密保存。"/><div className="status-band"><div><span>智能体状态</span><strong>{snapshot.agentStatus === "ready" ? "已就绪" : snapshot.agentStatus === "degraded" ? "降级运行" : "尚未配置"}</strong><small>{snapshot.agentCapabilities.toolCalling ? `工具调用可用${snapshot.agentCapabilities.vision ? "，识图工具可用" : ""}` : "陪伴聊天可用时，待办、识图与自动记忆工具将停用"}</small></div></div><form className="form-grid model-form" action={(form) => void saveModel(form)}><h2>聊天模型</h2><Field label="Chat API Base URL" wide><input name="baseUrl" defaultValue={snapshot.model.baseUrl} placeholder="https://api.openai.com/v1"/></Field><Field label="聊天模型"><input name="model" defaultValue={snapshot.model.model}/></Field><Field label="Temperature"><input name="temperature" type="number" min="0" max="2" step="0.1" defaultValue={snapshot.model.temperature}/></Field><Field label="Chat API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.model.configured ? "已安全保存，留空则不修改" : "输入 API Key"}/></Field><div className="form-actions"><button className="primary"><Save size={17}/>保存连接</button><button type="button" className="secondary" onClick={() => void window.everby.testModel().then((result) => setStatus(result.message))}>探测能力</button></div></form><form className="form-grid model-form" action={(form) => void saveVision(form)}><h2>图片理解模型</h2><p className="form-description">作为聊天智能体的 inspect_image 工具，仅在本轮附加图片时按需调用。</p><Field label="Vision API Base URL" wide><input name="baseUrl" defaultValue={snapshot.vision.baseUrl}/></Field><Field label="视觉模型"><input name="model" defaultValue={snapshot.vision.model}/></Field><Field label="Vision API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.vision.configured ? "已安全保存，留空则不修改" : "输入独立 API Key"}/></Field><div className="form-actions"><button className="primary"><Save size={17}/>保存图片理解模型</button><button type="button" className="secondary" onClick={() => void window.everby.testVision().then((result) => setStatus(result.message))}>测试识图</button></div></form><form className="form-grid model-form" action={(form) => void saveEmbedding(form)}><h2>Embedding 模型</h2><Field label="Embedding API Base URL" wide><input name="baseUrl" defaultValue={snapshot.embedding.baseUrl}/></Field><Field label="Embedding 模型"><input name="model" defaultValue={snapshot.embedding.model}/></Field><Field label="Embedding API Key" wide><input name="apiKey" type="password" autoComplete="off" placeholder={snapshot.embedding.configured ? "已安全保存，留空则不修改" : "输入独立 API Key"}/></Field><button className="primary form-submit"><Save size={17}/>保存 Embedding</button></form></section>}
