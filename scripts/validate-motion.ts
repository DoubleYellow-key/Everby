...
const directory = await mkdtemp(join(tmpdir(), "everby-motion-"));
try {
  const result = await new MotionService(directory).install(resolve(archivePath), new Set(["idle", "run-right", "run-left", "wave", "jump", "failed", "stretch", "working", "review", "drag"]));
