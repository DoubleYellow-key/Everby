...
**方向硬约束:行 1 = 角色朝右走、行 2 = 角色朝左走。** 提示词里必须写明朝向,预览时逐张核对——镜像装反是最常见的返工原因,组装进图集后才发现就得整行重来。

### 第一步:锁定基准形象

先只生成一张基准图 `base.png`:中立站姿全身像、纯色幕底(亮绿 `#00ff00` 或品红)、脚底对齐画面底边。如果用户提供的参考图本身姿势中立、适合当基准,直接用它。

**必须等用户确认基准形象(脸、发型、服装、比例)之后,才开始生成任何动画帧。** 基准没锁定就往下走,后面 9 行全都可能返工。基准图是之后每一帧的共同参考。

### 第二步:逐行链条式生成

- 每次只生成一行,保存为 `<工作目录>/row<行号>/<列号,从0开始>.png`。
- **链条式参考(实测帧间差更小、动画更顺滑)**:每行第 0 帧以 `base.png` 为参考生成;从第 1 帧起,每帧同时附上 `base.png`(锁形象)和本行的上一帧(锁连续性),要求"同一角色、同一构图,只推进动作相位",不要让每帧从基准图重新发挥。
- **固定姿势的动作(挥手、伸展、工作、检查等)先锁锚点帧**:这类动作整行是同一姿势的细微变化,先生成该行姿势的锚点帧给用户确认,确认后整行都从锚点帧派生(此时锚点帧替代基准图承担"上一帧"的角色)。
- 每行生成后给用户看预览,外观、动作、朝向不对就重生成该行;一致性靠逐行确认收敛,不要一次生成全部 9 行。
