...
const api: EverbyApi = {
  ...
  updateVision: (patch) => ipcRenderer.invoke("vision:update", patch),
  testModel: () => ipcRenderer.invoke("model:test"),
  testVision: () => ipcRenderer.invoke("vision:test"),
  selectChatImages: () => ipcRenderer.invoke("chat:select-images"),
  prepareChatImages: (images) => ipcRenderer.invoke("chat:prepare-images", images),
  sendMessage: (content, attachments = []) => ipcRenderer.invoke("chat:send", content, attachments),
